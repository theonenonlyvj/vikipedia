...
- Players can create new challenges by entering start and target article titles;
  authenticated server routes assign the next `Challenge #N` number.
...
- Runs, clicks, path steps, and leaderboard rows are written through
  Cloudflare Pages Functions to D1, not localStorage.
